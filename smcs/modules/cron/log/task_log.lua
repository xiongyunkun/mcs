...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/task.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Urs", "AchieveId", "FinishLevel"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = TaskLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
			...
			for PlatformID, Results in pairs(PlatformResults) do
				TaskLogData:BatchInsert(PlatformID, Results)
			end
