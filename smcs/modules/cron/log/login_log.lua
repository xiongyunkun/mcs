...
-- local Cols = {"Uid", "Urs", "Ip", "Name", "Sex", "Lv"}
local Cols = {"Uid", "Name", "Urs", "Level", "Ip", "Sex", "PhoneInfo"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = LoginLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
			...
			for PlatformID, Results in pairs(PlatformResults) do
				LoginLogData:BatchInsert(PlatformID, Results)
				--同时更新用户信息表
				UserInfoData:BatchInsert(PlatformID, Results)
			end
