...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/pet.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Urs", "Name", "OperationType", "PetID", "NeedResource", "Result", 
	"PetUid", "Level", "StarLevel", "Fighting"}
...
function HandleResponse(self, PlatformID, HostID, Response, ExecuteTime, LastSaticsTimes)
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			local SameTimeStatics = PetLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			local Results = {}
			...
			if #Results > 0 then
				PetLogData:BatchInsert(PlatformID, Results)
			end
