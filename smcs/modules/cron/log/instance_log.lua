----------------------------------------
--$Id: instance_log.lua 4082 2014-05-19 02:44:03Z zork $
----------------------------------------
--[[
-- 拉取instance.log相关日志并且入库,相关日志文件有：additem.log、subitem.log
--]]

module(...,package.seeall)

--统计指标
IndexName = "InstanceLog" 
--请求参数类型
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/instance.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Name", "InstanceID", "InstanceName", "OperationType", "Num",
	 "Result", "Reward", "Star",}
...
function HandleResponse(self, PlatformID, HostID, Response, ExecuteTime, LastSaticsTimes)
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			local SameTimeStatics = InstanceLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			local Results = {}
			...
			if #Results > 0 then
				InstanceLogData:BatchInsert(PlatformID, Results)
			end
...
--获得操作类型
function GetOperationType(self, FileName)
	local OperationTypes = {
		["additem.log"] = 1, 
		["subitem.log"] = 2,
	}
	return OperationTypes[FileName]
end
