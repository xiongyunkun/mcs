...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/stage.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Name", "Urs", "StageId", "StageType","Times", "NotPassEvent", "Bonus", 
	"IsFinish", "CardList", "IsFirstWin", "IsFirstEnter"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = InstanceLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
