...
----------------------------------------
--[[
-- 拉取mountlevel.log相关日志并且入库,相关日志文件有：mountlevel.log
-- 这里拉取的日志不是一条一条记录入库的，而是会根据主键Uid覆盖之前的日志的，因为
-- 不需要按照日志来统计
--]]
...
--统计指标
IndexName = "MountLevelLog" 
--请求参数类型
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/mountlevel.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "RegTime", "LoginTime", "UpdateTime", "MountID", "EvoLevel", "Level"}
