...
local Cols = {"Uid", "Urs", "Name", "ShopId", "ItemType", "Amount", "CostType" ,"CostValue"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = ShopBuyLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
