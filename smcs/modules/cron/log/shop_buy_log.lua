...
--统计指标
IndexName = "ShopBuyLog" 
--请求参数类型
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/shopbuy.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "ShopID", "ItemID", "ItemType", "ItemName", "Currency", "Amount", "Price", "Memo", "Time"}
...
function HandleResponse(self, PlatformID, HostID, Response, ExecuteTime, LastSaticsTimes)
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			local SameTimeStatics = ShopBuyLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			local Results = {}
			...
			if #Results > 0 then
				ShopBuyLogData:BatchInsert(PlatformID, Results)
			end
