...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/message.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "SenderName", "MessageType", "Title", "Content", "Bonus",}
...
function HandleResponse(self, PlatformID, HostID, Response, ExecuteTime, LastSaticsTimes)
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			local SameTimeStatics = MessageLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			local Results = {}
