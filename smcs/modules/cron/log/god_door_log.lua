----------------------------------------
--$Id: god_door_log.lua 4082 2014-05-19 02:44:03Z zork $
----------------------------------------
--[[
-- 拉取goddoor.log相关日志并且入库,相关日志文件有：goddoor.log
--]]

module(...,package.seeall)

--统计指标
IndexName = "GodDoorLog"
--请求参数类型
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/goddoor.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Urs", "Name", "DoorType", "SubType", "Reward",}
...
function HandleResponse(self, PlatformID, HostID, Response, ExecuteTime, LastSaticsTimes)
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			local SameTimeStatics = GodDoorLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			local Results = {}
			...
			if #Results > 0 then
				GodDoorLogData:BatchInsert(PlatformID, Results)
			end
