...
--统计指标
IndexName = "LuckyDrawLog" 
--请求参数类型
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/luckydraw.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Urs", "Name","Times", "ProcType", "Bonus",}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = LuckyDrawLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
			...
			for PlatformID, Results in pairs(PlatformResults) do
				LuckyDrawLogData:BatchInsert(PlatformID, Results)
			end
