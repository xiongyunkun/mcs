----------------------------------------
--$Id: power_log.lua 88008 2016-01-18 12:51:31Z xiongyunkun $
----------------------------------------
--[[
...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/addpower.log", "key/subpower.log"}

local Cols = {"Uid", "Urs", "DPower", "Reason", "Power", "Name"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = PowerLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
			...
			for PlatformID, Results in pairs(PlatformResults) do
				PowerLogData:BatchInsert(PlatformID, Results)
			end
