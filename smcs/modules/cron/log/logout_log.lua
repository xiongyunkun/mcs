...
local Cols = {"Uid", "Urs", "OnTime", "Level", "VipLevel", "IsVip", "Name", "Gold", "TotalGold", 
	"Money", "Ip", "Fighting"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = LogoutLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
