...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/additem.log", "key/subitem.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Name", "OperationType", "Type", "Reason", "Memo", 
	"ItemName", "ItemType", "Amount", "ItemUid"}
...
function HandleResponse(self, HostID, Response, ExecuteTime, LastSaticsTimes)
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			local LastTime = nil
			local PlatformIDs = {}
			local OperationType = self:GetOperationType(FileName)
				...
				if Line ~= "" and Line ~= " " then
					...
					for _, Col in ipairs(Cols) do
						...
						if Col == "OperationType" then
							ColValue = OperationType
						else
							ColValue = CommonFunc.GetLogValue(Line, Col)
						end
						Result[Col] = ColValue
					end
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = ItemLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
