...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/addgold.log", "key/subgold.log", "key/addcreditgold.log", "key/subcreditgold.log"}
...
function HandleResponse(self, PlatformID, HostID, Response, ExecuteTime, LastSaticsTimes)
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			local SameTimeStatics = GoldLogData:GetSameTimeStatics(PlatformID,{HostID=HostID, Time=LastSaticsTime})
			local Results = {}
			local Lines = string.split(LogContent, "\n")
			local LastTime = nil
			--根据文件名确定钻石类型
			local GoldType = ""
			if string.find(FileName, "creditgold") then
				GoldType = "2" --绑定钻石
			else
				GoldType = "1" --非绑定钻石
			end
