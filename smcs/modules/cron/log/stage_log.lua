...
----------------------------------------
--[[
-- 拉取stage.log相关日志并且入库,相关日志文件有：stage.log
--]]
...
local Cols = {"Uid", "Urs", "Name","StageId", "CardList", "StageType", "Times", 
	"IsFinish", "Bonus", "IsFirstWin"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = StageLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
			...
			for PlatformID, Results in pairs(PlatformResults) do
				StageLogData:BatchInsert(PlatformID, Results)
			end
