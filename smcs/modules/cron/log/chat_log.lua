----------------------------------------
--$Id: chat_log.lua 4082 2014-05-19 02:44:03Z zork $
----------------------------------------
--[[
-- 拉取chat.log相关日志并且入库,相关日志文件有：chat.log
--]]
...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/chat.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Name", "ChannelID", "Msg", "MD5"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = ChatLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
			...
			for PlatformID, Results in pairs(PlatformResults) do
				ChatLogData:BatchInsert(PlatformID, Results)
			end
