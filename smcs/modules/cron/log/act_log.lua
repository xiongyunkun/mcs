...
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/act.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Urs", "ActName", "Status", "Time"}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = ActLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
			local PlatformResults = {}
			for _, Result in ipairs(Results) do
				if Result["PlatformID"] and SameTimeStatics[Result["PlatformID"]] 
					and SameTimeStatics[Result["PlatformID"]][UniqueValue] ~= Result["Time"] then
			...
			for PlatformID, Results in pairs(PlatformResults) do
				ActLogData:BatchInsert(PlatformID, Results)
			end
