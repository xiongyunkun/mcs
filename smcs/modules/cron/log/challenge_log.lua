----------------------------------------
--$Id: lucky_draw_log.lua 4082 2014-05-19 02:44:03Z zork $
----------------------------------------
--[[
-- 拉取luckydraw.log相关日志并且入库,相关日志文件有：luckydraw.log
--]]

module(...,package.seeall)

--统计指标
IndexName = "ChallengeLog" 
--请求参数类型
RequestType = "getlog"
-- 需要读取的文件名
FileNames = {"key/etchallenge.log"}

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Urs", "Name","Level", "ChapterId", "Idx", "StageId", "IsWin",}
	...
	for FileName, LogContent in pairs(Response) do
		if LogContent ~= "" or LogContent ~= " " then
			...
			for PlatformID, _ in pairs(PlatformIDs) do
				SameTimeStatics[PlatformID] = ChallengeLogData:GetSameTimeStatics(PlatformID,{HostID=HostID,Time=LastSaticsTime})
			end
			...
			for PlatformID, Results in pairs(PlatformResults) do
				ChallengeLogData:BatchInsert(PlatformID, Results)
			end
