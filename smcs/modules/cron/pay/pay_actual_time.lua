...
--获得上一次统计时间
function GetLastStaticsTime(self, PlatformID, HostID)
	local OrderIDs = ""
	local StartDate = os.date("%Y-%m-%d",ngx.time()) 
	local StaticsTime = os.date("%Y-%m-%d %H:%M:%S",GetTimeStamp(StartDate .. " 00:00:00"))  --默认是当天的0点
	local Res = PayStaticsCfgData:Get({PlatformID = PlatformID, HostID = HostID})
...
function CronExecute(self, HostID)
	local PRes = MixServerData:Get({HostID = HostID})
	for _, PInfo in ipairs(PRes) do
		local PlatformID = PInfo.PlatformID
		local LastStaticsTime, OrderIDs = self:GetLastStaticsTime(PlatformID, HostID)
		...
		if NowStaticsTime then
			--更新统计时间
			PayStaticsCfgData:Update({PlatformID = PlatformID, HostID = HostID, OrderIDs = table.concat( NowOrderIDs, ","), StaticsTime = NowStaticsTime})
		end
	end
end
