----------------------------------------
--$Id: phone_retention.lua 88008 2016-01-18 12:51:31Z xiongyunkun $
----------------------------------------
--[[
-- 留存率统计,针对新注册用户的指标统计
--]]
...
function GetRegData(self, PlatformID, HostID, Date)
	...
	for _, RegInfo in ipairs(RegRes) do
		local PhoneInfo = RegInfo.PhoneInfo
	...
	for _, LoginInfo in ipairs(LoginRes) do
		local PhoneInfo = LoginInfo.PhoneInfo
		local PhoneInfoTbl = string.split(PhoneInfo, ";")
		local IMEI = PhoneInfoTbl[8] 
		if IMEI then
			LoginList[IMEI] = true
	...
	local Results = {}
	for IMEI, _ in pairs(ResList) do
		if LoginList[IMEI] then
			Results[IMEI] = true
		end
	end
	return Results
...
function CronStatics(self, PlatformID, HostID)
	...
	PhoneRetentionData:Insert(PlatformID, HostID, Today, Temp)
	PhoneRetentionData:Insert(PlatformID, HostID, Day1, {["1Days"] = Rate1})
	PhoneRetentionData:Insert(PlatformID, HostID, Day2, {["2Days"] = Rate2})
	PhoneRetentionData:Insert(PlatformID, HostID, Day3, {["3Days"] = Rate3})
	PhoneRetentionData:Insert(PlatformID, HostID, Day4, {["4Days"] = Rate4})
	PhoneRetentionData:Insert(PlatformID, HostID, Day5, {["5Days"] = Rate5})
	PhoneRetentionData:Insert(PlatformID, HostID, Day6, {["6Days"] = Rate6})
	PhoneRetentionData:Insert(PlatformID, HostID, Day7, {["7Days"] = Rate7})
	PhoneRetentionData:Insert(PlatformID, HostID, Day10, {["10Days"] = Rate10})
	PhoneRetentionData:Insert(PlatformID, HostID, Day13, {["13Days"] = Rate13})
	PhoneRetentionData:Insert(PlatformID, HostID, Day15, {["15Days"] = Rate15})
	PhoneRetentionData:Insert(PlatformID, HostID, Day29, {["29Days"] = Rate29})
	PhoneRetentionData:Insert(PlatformID, HostID, Day30, {["30Days"] = Rate30})
	return true
end
...
function GetRetenRate(self, RegList, LoginList)
	...
	local RetNum = 0
	for IMEI, _ in pairs(LoginList) do
		if RegList[IMEI] then
			RetNum = RetNum + 1
		end
	end
