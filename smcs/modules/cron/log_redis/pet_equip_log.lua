...
----------------------------------------
--[[
-- 拉取petequip.log相关日志并且入库,相关日志文件有：petequip.log
--]]

module(...,package.seeall)

--前面的HostID和后面的时间都不要在这里填
local Cols = {"Uid", "Urs", "Name", "Position", "Consume", "OperationType", "NewItemName"}
...
function HandleResponse(self, LogList, ServerPlatformMap)
	...
	for PlatformID, Results in pairs(PlatformResults) do
		PetEquipLogData:BatchInsert(PlatformID, Results)
	end
	return true
end
