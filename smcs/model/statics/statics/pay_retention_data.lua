...
function MergeData(self, OrgResults, TotalResults)
	for _, Info in ipairs(OrgResults) do
		if not TotalResults[Info.Date] then
			...
			DateInfo.FirstPayUserNum = DateInfo.FirstPayUserNum + Info.FirstPayUserNum
...
function GetRetRate(self, OrgDateInfo, NewDateInfo, Index)
	local FirstPayUserNum = OrgDateInfo.FirstPayUserNum + NewDateInfo.FirstPayUserNum
	if FirstPayUserNum == 0 then
		return 0
	else
		local OrgRetNum = OrgDateInfo[Index] * OrgDateInfo.FirstPayUserNum * 100
		local NewRetNum = NewDateInfo[Index] * NewDateInfo.FirstPayUserNum * 100
		local C = math.floor((OrgRetNum + NewRetNum)/FirstPayUserNum)
		C = C/100
		return C
	end
end

local Cols = {"LoginNum","FirstPayUserNum","1Days", "2Days", "3Days", "4Days","5Days","6Days", "7Days", "10Days", 
	"13Days", "15Days", "29Days", "30Days"}

function Insert(self, PlatformID, HostID, Date, RetInfo)
	local Sql = "insert into " .. PlatformID .. "_statics.tblPayRetention(PlatformID, HostID, Date,"..table.concat(Cols, ",")
