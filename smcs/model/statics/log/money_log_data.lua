...
function Get(self, PlatformID, Options)
	...
	local Sql = "select * from " .. PlatformID .. "_log.tblMoneyLog " .. Where
...
local Cols = {"HostID", "Uid", "Urs", "Changes", "Reason", "Remain", "Name", "Time"}
function BatchInsert(self, PlatformID, Results)
	...
	--插入数据库
	local Sql = "insert into " .. PlatformID .. "_log.tblMoneyLog(" .. table.concat(Cols, ",") .. ") values("
	-- 采用批量插入的方式
