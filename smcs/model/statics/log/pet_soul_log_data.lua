...
function Get(self, PlatformID, Options)
	...
	if Options.OperationType and Options.OperationType ~= "0" then
    ...
    for _, Date in ipairs(DateList) do
		local Sql = "select * from " .. PlatformID .. "_log.tblPetSoulLog_" .. Date .. Where
...
local Cols = {"HostID", "Uid", "Urs", "Name", "OperationType", "Grid", "Consume", "ItemType",
	 "Amount", "Time"}
...
function BatchInsert(self, PlatformID, Results)
	...
	for Date, DateResults in pairs(StrResults) do
		local Sql = "insert into " .. PlatformID .. "_log.tblPetSoulLog_" .. Date .. "(" 
